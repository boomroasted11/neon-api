...
// ✅ Health / alive routes (helps Railway / load balancers)
...
app.post('/insert', async (req, res) => {
  ...
  let { date, description, category, amount, currency, type } = req.body
  const isHtmlUploader = String(req.header('X-Client') || '').toLowerCase() === 'html-uploader'

  // ✅ Robust amount handling (accept number or numeric string)
  const amountNum = typeof amount === 'number' ? amount : Number(amount)

  if (!date || !description || Number.isNaN(amountNum)) {
  ...
  try {
    if (table === 'transactions') {
      if (isHtmlUploader) {
        const key = makeKey({ date, description, amount: amountNum })
        ...
        await pool.query(
          `INSERT INTO transactions (date, description, category, amount, currency, type)
           VALUES ($1,$2,$3,$4,$5,$6)`,
          [date, description, category, amountNum, currency || 'USD', type]
        )
    ...
    if (table === 'crosspoint') {
      ...
      await pool.query(
        `INSERT INTO crosspoint (date, description, category, amount, type)
         VALUES ($1,$2,$3,$4,$5)`,
        [date, description, category, amountNum, type]
      )
